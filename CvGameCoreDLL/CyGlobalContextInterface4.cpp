...
#include "CvInfos.h"
#include "CyTeam.h"
#include "CyRiseFall.h" //bluepotato
...
void CyGlobalContextPythonInterface4(python::class_<CyGlobalContext>& x)
{
	...
	x
		...
		//bluepotato start
		.def("setMinorNationCiv", &CyGlobalContext::setMinorNationCiv, "void (int eID, int civType, bool bMinorNationCiv)")
		.def("onCivSelectionScreenLoaded", &CyGlobalContext::onCivSelectionScreenLoaded, "void ()")
		.def("getRiseFall", &CyGlobalContext::getCyRiseFall, python::return_value_policy<python::reference_existing_object>(), "()")
		.def("setupEnabled", &CyGlobalContext::setupEnabled, "void ()")
		//bluepotato end
		;
}
