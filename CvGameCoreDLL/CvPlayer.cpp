...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bTrade, bool bUpdatePlotGroups)
{
	...
	for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
	{
		...
		if (pabHolyCity[iI])
		{
			...
			for (int j = 0; j < GC.getMAX_PLAYERS(); j++) {
				...
				{
					GET_PLAYER(getID()).AI_invalidateAttitudeCache((PlayerTypes)j);
	...
	if (bConquest)
	{
		...
		{
			//bluepotato start: Babylonian UP
			if(getCivilizationType() != CIVILIZATION_BABYLON) {
		...
		if(getCivilizationType() == CIVILIZATION_MACEDONIA) {
			for(int x = pNewCity->getX()-2; x <= pNewCity->getX()+2; ++x) {
				for(int y = pNewCity->getY()-2; y <= pNewCity->getY()+2; ++y) {
					...
					if(percent!=-1) {
						int convertedCulture = 0;
						for(int i = 0; i<MAX_PLAYERS; ++i) {
							if(pNewCity->getCulture((PlayerTypes)i) > 0 && i!=getID()) {
								int toConvert = pNewCity->getCulture((PlayerTypes)i)*(100-percent)/100;
								convertedCulture += toConvert;
								pNewCity->setCulture((PlayerTypes)i, toConvert, true, true);
							}
						}
						pNewCity->changeCulture(getID(), convertedCulture, true, true);
						convertedCulture = 0;
						for(int i = 0; i<MAX_PLAYERS; ++i) {
							if(i != getID()) {
								int toConvert = loopPlot->getCulture((PlayerTypes)i)*(100-percent)/100;
								convertedCulture += toConvert;
								loopPlot->setCulture((PlayerTypes)i, toConvert, true, true);
							}
						}
						loopPlot->changeCulture(getID(), convertedCulture, true);
					}
				}
			}
		}

		//Bactrian UP (converted from Python)
		if(getCivilizationType()==CIVILIZATION_BACTRIA) {
			SpecialistTypes artist = (SpecialistTypes)GC.getInfoTypeForString("SPECIALIST_ARTIST");
			getCapitalCity()->changeFreeSpecialistCount(artist, 1);
		}

		//Byzantine UP (converted from Python)
		if(getCivilizationType()==CIVILIZATION_BYZANTIUM) {
			if (getStateReligion() == 1) { //TODO: this should work with all religions
				if(!pNewCity->isHasReligion(getStateReligion())) {
					pNewCity->setHasReligion(getStateReligion(), true, false);
				}
				if(pNewCity->getNumBuilding((BuildingTypes)GC.getInfoTypeForString("BUILDING_CHRISTIAN_TEMPLE"))==0) {
					pNewCity->setNumRealBuilding((BuildingTypes)GC.getInfoTypeForString("BUILDING_CHRISTIAN_TEMPLE"), 1);
				}
				if(pNewCity->getNumBuilding((BuildingTypes)GC.getInfoTypeForString("BUILDING_CHRISTIAN_CATHEDRAL"))==0) {
					pNewCity->setNumRealBuilding((BuildingTypes)GC.getInfoTypeForString("BUILDING_CHRISTIAN_CATHEDRAL"), 1);
				}
			}
		}
		//bluepotato end
	...
	//bluepotato start
	processDynamicNames();
	GET_PLAYER(eOldOwner).processDynamicNames();
	//bluepotato end
}
...
CvUnit* CvPlayer::initUnit(UnitTypes eUnit, int iX, int iY, UnitAITypes eUnitAI, DirectionTypes eFacingDirection, int maintenanceCost) //bluepotato: merc maintenance costs
{
	PROFILE_FUNC();
	...
	if (NULL != pUnit)
	{
		pUnit->init(pUnit->getID(), eUnit, ((eUnitAI == NO_UNITAI) ? ((UnitAITypes)(GC.getUnitInfo(eUnit).getDefaultUnitAIType())) : eUnitAI), getID(), iX, iY, eFacingDirection, maintenanceCost); //bluepotato: merc maintenance costs
	}
...
{
	//bluepotato start: dynamic names (from Rhye)
...
bool CvPlayer::canRaze(CvCity* pCity) const
{
	if (!pCity->isAutoRaze())
	{
		...
		{
			if(getCivilizationType() != CIVILIZATION_MACEDONIA) //bluepotato
				return false;
		}
	}
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	//bluepotato: this was triggered by platybuilder, disabled because there's no harm in it + it's annoying
...
int CvPlayer::getProductionNeeded(UnitTypes eUnit) const
{
	...
	//bluepotato start: unit production modifiers
	/*
	...
	iProductionNeeded *= 12*getCurrentEra() + 5 + 100; //105-117-129
	iProductionNeeded /= 100;

	if (GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_LAND) {
		iProductionNeeded *= 100 + 2*getCurrentEra(); //100-107-121
		iProductionNeeded /= 100;
	}
	//bluepotato end
...
/************************************************************************************************/
	...
	//bluepotato start: Egyptian UP
	if(getCivilizationType() == CIVILIZATION_EGYPT) {
		if ((eCivic == 1) || (eCivic == 11) || (eCivic == 21)) {
			return true;
		}
	}
	//bluepotato end
...
{
	//bluepotato start: minor civs can't convert
...
void CvPlayer::foundReligion(ReligionTypes eReligion, ReligionTypes eSlotReligion, bool bAward)
{
	...
	{
		//bluepotato start: fix religion founding
		/*
		if (isHuman())
		{
...
void CvPlayer::changeGoldenAgeTurns(int iChange)
{
	...
	if (iChange != 0)
	{
		...
		if (bOldGoldenAge != isGoldenAge())
		{
			...
			if (isGoldenAge())
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_GOLDEN_AGE_BEGINS", getCivilizationAdjectiveKey()); //bluepotato: civ adjective instead of player name
			...
			for (iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					if (GET_TEAM(getTeam()).isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam()))
					{
						if (isGoldenAge())
						{
							szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_GOLDEN_AGE_HAS_BEGUN", getCivilizationAdjectiveKey()); //bluepotato: civ adjective instead of player name
						...
						{
							szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_GOLDEN_AGE_ENDED", getCivilizationAdjectiveKey());
...
{
	//bluepotato start: Indian UP
	if(getCivilizationType()==CIVILIZATION_INDIA) {
		m_iMaxAnarchyTurns = 0;
		return;
	}
	//bluepotato end
...
void CvPlayer::setCombatExperience(int iExperience)
{
	...
	if (iExperience != getCombatExperience())
	{
		...
		if (!isBarbarian() && !isMinorCiv()) //bluepotato: no minor great generals
		{
				...
				if (getLastStateReligion() != NO_RELIGION)
				{
					for (iI = 0; iI < MAX_PLAYERS; iI++)
					{
						if (GET_PLAYER((PlayerTypes)iI).isAlive())
						{
							if (GET_TEAM(getTeam()).isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam()))
							{
								szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_CONVERT_RELIGION", getCivilizationShortDescriptionKey(), GC.getReligionInfo(getLastStateReligion()).getTextKeyWide()); //bluepotato: civ desc instead of player name
...
void CvPlayer::setCivics(CivicOptionTypes eIndex, CivicTypes eNewValue, bool startingCivics) //bluepotato: starting civics bool
{
	CvWString szBuffer;
	...
	if (eOldCivic != eNewValue)
	{
		...
		if (GC.getGameINLINE().isFinalInitialized())
		{
			...
			if (!isBarbarian() && !startingCivics && !isMinorCiv()) //bluepotato: do not show starting civics + do not show civic changes for indies
			{
				if (getCivics(eIndex) != NO_CIVIC)
				{
					if (getCivics(eIndex) != GC.getCivilizationInfo(getCivilizationType()).getCivilizationInitialCivics(eIndex))
					{
						for (iI = 0; iI < MAX_PLAYERS; iI++)
						{
							if (GET_PLAYER((PlayerTypes)iI).isAlive())
							{
								if (GET_TEAM(getTeam()).isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam()))
								{
									szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_ADOPTED_CIVIC", getCivilizationShortDescriptionKey(), GC.getCivicInfo(getCivics(eIndex)).getTextKeyWide()); //bluepotato: civ desc instead of player name
...
bool CvPlayer::canDoEspionageMission(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pUnit) const
{
	...
	//bluepotato start, from Rhye
	if(GET_PLAYER(eTargetPlayer).isMinorCiv() || GET_PLAYER(eTargetPlayer).isBarbarian()) {
		return false;
	}
	//bluepotato end
...
void CvPlayer::processDynamicNames(bool force) {
	...
	for(int i = 0; i < MAX_CIV_PLAYERS; i++) {
		CvPlayer& loopPlayer = GET_PLAYER((PlayerTypes)i);
		if(loopPlayer.isAlive() && loopPlayer.getTeam()!=getTeam()) {
			if(GET_TEAM(getTeam()).isVassal(loopPlayer.getTeam())) {
				masterType = (PlayerTypes)i;
				break;
			}
		}
	}
	...
	if (masterType != NO_PLAYER) {
		...
		if(master.getCivilizationType() == CIVILIZATION_PERSIA) {
			o << L"08";
		} else if(master.getCivilizationType() == CIVILIZATION_ROME) {
			o << L"09";
		} else {
			o << L"10";
		}
		...
		if(getNumCities() == 0) {
			...
			o << L"03";
		} else if(getNumCities() <= 8) {
			o << L"04";
		} else if(getNumCities() <= 12) {
			o << L"05";
		} else if(getNumCities() <= 16) {
			o << L"06";
		} else {
			o << L"07";
		}
