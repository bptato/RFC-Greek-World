...
void CvUnit::init(int iID, UnitTypes eUnit, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, int newMaintenanceCost)
{
	CvWString szBuffer;
	int iUnitName;
	int iI, iJ;
	...
	GET_PLAYER(getOwnerINLINE()).changeExtraUnitCost(maintenanceCost);
	//bluepotato end
...
void CvUnit::kill(bool bDelay, PlayerTypes ePlayer)
{
	...
	GET_PLAYER(getOwnerINLINE()).changeExtraUnitCost(-(m_pUnitInfo->getExtraCost()));
	GET_PLAYER(getOwnerINLINE()).changeExtraUnitCost(-maintenanceCost); //bluepotato
...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (bFinish)
	{
		...
		else if (pDefender->isDead())
		{
			...
			//bluepotato start: Assyrian UP
			UnitTypes defUnitType = pDefender->getUnitType();
			bool defUnitAnimal = pDefender->isAnimal();
			PlayerTypes defUnitOwner = pDefender->getOwnerINLINE();
			//bluepotato end
			...
			//bluepotato start: Assyrian UP
			//placed after movement cause enemy units would be somehow preserved otherwise
...
bool CvUnit::pillage()
{
	...
	if (pPlot->getImprovementType() != NO_IMPROVEMENT)
	{
		...
		if (pPlot->getTeam() != getTeam())
		{
			...
			//bluepotato start: Hunnic UP
			if(getCivilizationType() == CIVILIZATION_HUNS) {
				iPillageGold *= 5;
			}
			//bluepotato end
			if (iPillageGold > 0)
	...
	lastAction = GC.getGame().getGameTurn(); //bluepotato
	return true;
}
...
int CvUnit::upgradePrice(UnitTypes eUnit) const
{
	...
	//bluepotato start: Spartan UP
	if (getCivilizationType() == CIVILIZATION_SPARTA) {
		static TechTypes maths = (TechTypes)GC.getInfoTypeForString("TECH_MATHEMATICS");
		if(GET_TEAM(GET_PLAYER(getOwnerINLINE()).getTeam()).isHasTech(maths)) {
			return 0;
		}
	}
	//bluepotato end
...
{
	//bluepotato start: speed (ported from Python)
	/*// Use python to determine pillage amounts...
	...
	//Note from K-Mod: python rounds towards negative infinity, but C++ rounds towards 0.
	//So the code needs to be slightly different to achieve the same effect.
	int experienceNeeded = getLevel() * getLevel() + 1;
	int modifier = GET_PLAYER(getOwnerINLINE()).getLevelExperienceModifier();
	if(modifier != 0) {
		experienceNeeded = (experienceNeeded * (100 + modifier) + 99) / 100;
	}
...
void CvUnit::read(FDataStreamBase* pStream)
{
	...
	//bluepotato start
	pStream->Read(&lastAction);
	pStream->Read(&maintenanceCost);
	//bluepotato end
...
void CvUnit::write(FDataStreamBase* pStream)
{
	...
	//bluepotato start
	pStream->Write(lastAction);
	pStream->Write(maintenanceCost);
	//bluepotato end
...
//bluepotato start
int CvUnit::getLastAction() {
	return lastAction;
}

int CvUnit::getMaintenanceCost() {
