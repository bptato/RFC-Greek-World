...
/************************************************************************************************/
	...
	//bluepotato start: settler maps
	int settlerValue = pPlot->getSettlerValue(getCivilizationType());
	iValue *= settlerValue;
	iValue /= 100;

	if(settlerValue <= 3) {
		iValue /= 4;
	} else if(settlerValue <= 20) {
		iValue /= 3;
	} else if(settlerValue <= 60) {
		iValue /= 2;
	} else if(settlerValue >= 400) {
		iValue *= 12;
		iValue /= 10;
	} else if(settlerValue >= 200) {
		iValue *= 11;
		iValue /= 10;
	}
	//bluepotato end
...
/************************************************************************************************/
	...
	//bluepotato start: change target value based on settler maps (from Rhye)
	int settlerValue = pCity->plot()->getSettlerValue(getCivilizationType());
	if(settlerValue <= 3)
		iValue -= 2;
	else if(settlerValue <= 20)
		iValue -= 1;
	else if(settlerValue >= 500) //500-700
		iValue += 4;
	else if(settlerValue >= 300) //300-400
		iValue += 3;
	else if(settlerValue >= 150) //150-200
		iValue += 2;
	else if(settlerValue >= 40) //40-60-90
		iValue += 1;
	//bluepotato end
...
int CvPlayerAI::AI_cityTradeVal(CvCity* pCity) const
{
	...
	if (GET_PLAYER(pCity->getOwnerINLINE()).getNumCities() > 12) {
		if (pCity->plot()->getSettlerValue(getCivilizationType()) < 500) { //not in core area
			iValue *= 2;
			iValue /= 3;
		}
	}
												...
												if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_JOIN_WAR), "AI Diplo Join War") == 0)
												{
													...
													for (iJ = 0; iJ < MAX_CIV_TEAMS; iJ++)
													{
														if (GET_TEAM((TeamTypes)iJ).isAlive()
															//bluepotato start: do not ask to attack independents
				...
				if (pLoopUnit->getUnitInfo().getProductionCost() > 0)
				{
					if ((iExpThreshold == -1) || (pLoopUnit->canFight() && pLoopUnit->getExperience() <= iExpThreshold))
					{
						if (!(pLoopUnit->isMilitaryHappiness()) || !(pLoopUnit->plot()->isCity()) || (pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, -1, -1, getID()) > 2))
						{
							...
							//bluepotato start: mercenary disband logic
							if(pLoopUnit->getMaintenanceCost() > 0) {
								iValue /= pLoopUnit->getMaintenanceCost();
							}
							//bluepotato end
...
void CvPlayerAI::AI_doMercenaries() {
	//creative way to avoid calling this every turn
	if(GC.getGame().getGameTurn() % 6 != getID() % 6) {
		return;
	}
	...
	if(getGold() - getExtraUnitCost() < 0) {
		return;
	}

	//no cities, no mercenaries
	if(getNumCities() <= 0) {
		return;
	}

	//this should be based on leader personality but whatever
	...
	for(int i = 0; i<MAX_CIV_PLAYERS; ++i) {
		...
		if(player.isAlive()) {
			if(GET_TEAM(player.getTeam()).isAtWar(getTeam())) {
				++numEnemies;
				if(getPower() < player.getPower()) {
					++numMorePowerfulEnemies;
				}
			}
			if(getPower() < player.getPower()) {
				++numMorePowerfulCivs;
	...
	int mercenaryCount = numMorePowerfulEnemies * numEnemies + numMorePowerfulCivs;
	int randomMercenaryCount = GC.getGame().getSorenRandNum(mercenaryCount, "Mercenary hire amount roll");

	for(int i = 0; i<randomMercenaryCount; ++i) {
		//no mercs, no wasting computing power
		if(!AI_hireMercenary()) {
			return;
		}
	}
