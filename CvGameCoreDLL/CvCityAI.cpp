		...
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getTeam())
		{
			for (pLoopCity = GET_PLAYER((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
			{
				if (pLoopCity != this)
				{
					...
					{
						//bluepotato start: speed tweak, inspired by BTS on speed. But instead of making the resource intensive calculations random, we just use simple calculations.
						/*
						...
						iValue = abs(getX_INLINE() - pLoopCity->getX_INLINE()) + abs(getY_INLINE() - pLoopCity->getY_INLINE());
						if (iValue < iBestValue) {
							iBestValue = iValue;
							pBestCity = pLoopCity;
						}
