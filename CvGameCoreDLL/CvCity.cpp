...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits, bool bUpdatePlotGroups)
{
	...
	//bluepotato start: city names
	CvWString plotCityName(pPlot->getCityName(GET_PLAYER(eOwner).getCivilizationType()));

	if(!plotCityName.empty()) {
		setName(plotCityName);
	} else {
...
int CvCity::getGreatPeopleRate() const
{
	...
	//bluepotato start: Athenian UP
	int retval = ((getBaseGreatPeopleRate() * getTotalGreatPeopleRateModifier()) / 100);
	if (getCivilizationType() == CIVILIZATION_ATHENS) {
		retval *= 250;
		retval /= 100;
	}
...
void CvCity::setName(const wchar* szNewValue, bool bFound)
{
	...
	//gDLL->stripSpecialCharacters(szName); bluepotato: allows special characters in city names. borrowed from Rhye
...
/*************************************************************************************************/
			...
			//bluepotato start
			//Elamite UP
			if(getCivilizationType() == CIVILIZATION_ELAM && getBuildingCommerceChange((BuildingClassTypes)(GC.getBuildingInfo(eConstructBuilding).getBuildingClassType()), COMMERCE_CULTURE) > 0) {
				changeBuildingGoodHappiness(1);
			}
