...
void CvGameTextMgr::parseCivInfos(CvWStringBuffer &szInfoText, CivilizationTypes eCivilization, bool bDawnOfMan, bool bLinks)
{
	...
	if (eCivilization != NO_CIVILIZATION)
	{
		...
		if (bDawnOfMan)
		{
			...
			szTempString.Format(SETCOLR L"%s: " ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), szText.GetCString()); //bluepotato
		}
		else
		{
			szTempString.Format(NEWLINE SETCOLR L"%s" ENDCOLR , TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), szText.GetCString());
		...
		//bluepotato start: UPs and UHVs (from Rhye)
		...
		szTempString.Format(L"TXT_KEY_UHV_%s2", GC.getCivilizationInfo(eCivilization).getRFCID().c_str());
		szText += gDLL->getText(szTempString);
		szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_ICON_BULLET");
		szTempString.Format(L"TXT_KEY_UHV_%s3", GC.getCivilizationInfo(eCivilization).getRFCID().c_str());
		szText += gDLL->getText(szTempString);
