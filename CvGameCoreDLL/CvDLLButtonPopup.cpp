...
void CvDLLButtonPopup::OnOkClicked(CvPopup* pPopup, PopupReturn *pPopupReturn, CvPopupInfo &info)
{
	...
	case BUTTONPOPUP_CITY_NAME:
		{
			CivilizationTypes civType = (CivilizationTypes)info.getData1();
			int x = info.getData2();
			int y = info.getData3();
...
//bluepotato start: city name editing popup
bool CvDLLButtonPopup::launchCityNamePopup(CvPopup* popup, CvPopupInfo &info) {
	gDLL->getInterfaceIFace()->popupSetHeaderString(popup, gDLL->getText("TXT_KEY_POPUP_CITY_NAME_TITLE"));
	CivilizationTypes civType = (CivilizationTypes)info.getData1();
	int x = info.getData2();
	int y = info.getData3();
