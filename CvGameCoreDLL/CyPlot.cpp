...
//bluepotato start
void CyPlot::setCityName(int civType, std::wstring cityName) {
	if(m_pPlot)
		m_pPlot->setCityName((CivilizationTypes)civType, CvWString(cityName));
}
std::wstring CyPlot::getCityName(int civType, bool precise) {
	return m_pPlot ? m_pPlot->getCityName((CivilizationTypes)civType, precise) : L"";
}

short CyPlot::getSettlerValue(int civType) {
	return m_pPlot ? m_pPlot->getSettlerValue((CivilizationTypes)civType) : -1;
}

void CyPlot::setSettlerValue(int civType, short settlerValue) {
	if(m_pPlot)
