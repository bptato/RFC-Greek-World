...
import time
#bluepotato start
import CvMercenariesScreen

mercenaryScreen = CvMercenariesScreen.CvMercenariesScreen()
#bluepotato end
...
class CvMainInterface:
	...
	def updateMiscButtons( self ):
		...
		if ( CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL or CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_MINIMAP_ONLY ):
			...
			if gc.getActivePlayer().getNumCities() > 0:
				screen.show("MercenaryButton")
				screen.moveToFront("MercenaryButton")
			#bluepotato end
									...
									if (gc.getPlayer(ePlayer).getTeam() == eTeam):
										...
										if (not CyInterface().isFlashingPlayer(ePlayer) or CyInterface().shouldFlash(ePlayer)):
											if (ePlayer == gc.getGame().getActivePlayer()):
												szTempBuffer = u"%d: [<color=%d,%d,%d,%d>%s</color>]" %(gc.getGame().getPlayerScore(ePlayer), gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gc.getPlayer(ePlayer).getCivilizationDescription(0)) #bluepotato: civ desc instead of name
											else:
												szTempBuffer = u"%d: <color=%d,%d,%d,%d>%s</color>" %(gc.getGame().getPlayerScore(ePlayer), gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gc.getPlayer(ePlayer).getCivilizationDescription(0)) #bluepotato: civ desc instead of name
										else:
											szTempBuffer = u"%d: %s" %(gc.getGame().getPlayerScore(ePlayer), gc.getPlayer(ePlayer).getCivilizationDescription(0)) #bluepotato: civ desc instead of name
										szBuffer = szBuffer + szTempBuffer
										...
										#bluepotato start - stability (from Rhye)
										player = gc.getPlayer(ePlayer)
										if player.isAlive():
											if not player.isMinorCiv() and not player.isBarbarian():
												stability = CyRiseFall().getRFCPlayer(player.getCivilizationType()).getTotalStability()
												if (stability < -40):
	...
	def handleInput (self, inputClass):
		#bluepotato start: mercenaries
		if(inputClass.getFunctionName() == "MercenaryButton"):
			mercenaryScreen.interfaceScreen()
		#bluepotato end
		return 0
