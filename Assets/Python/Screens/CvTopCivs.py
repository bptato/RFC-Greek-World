...
#		iXHeaderPanelRemainingAfterLeader = self.X_LEADER_ICON + self.W_LEADER_ICON + self.iMarginSpace
	...
	def makeList(self, szType):
		...
		for iPlayerLoop in range(gc.getMAX_PLAYERS()-1):
			#bluepotato: no minors
			if gc.getPlayer(iPlayerLoop).isMinorCiv():
				continue
		...
		for iRankLoop in range(8):
			...
			if (szPlayerName != ""):
				...
				if (iPlayer == CyGame().getActivePlayer() or pActivePlayerTeam.isHasMet(iPlayerTeam)):
					#szCivText = localText.getText("TXT_KEY_TOPCIVS_TEXT3", (szPlayerName, self.RankList[iRankLoop])) #bluepotato (from Rhye)
					szCivText = gc.getPlayer(iPlayer).getCivilizationDescription(0)
				else:
